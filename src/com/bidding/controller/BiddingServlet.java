...
@WebServlet("/bidding/BiddingServlet")
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req.getRequestDispatcher("/bidding/biddingView.html");
		...
		if ("getOne".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/bidding/select_page.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				req.setAttribute("bVO", bVO);
				String url = "/bidding/listOne.jsp";
		...
		if ("getOneForUpdate".equals(action)) {
			...
			try {
				...
				req.setAttribute("bVO", bVO);
				String url = "update_bidding_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);

			} catch (Exception e) {
				errorMsgs.add("無法取得要修改的資料: " + e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/bidding/listAll.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("update".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("bVO", bVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/bidding/update_bidding_input.jsp");
					failureView.forward(req, res);
					return;
				}
				BiddingService bdSvc = new BiddingService();
